...
<<<<<<< HEAD
        ...
        public function actionsupervisor(){
            ...
            if ($session['user_role']!='1')
                $subordinates = $repo->getSupervisor($session['empnumber']);
            else
                $subordinates = $repo->getSupervisor($_REQUEST['emp_number']);
                 ...
                 {   
                     //leave_approval
                        ...
                        $editurl = Yii::app()->request->baseUrl."/index.php?r=Manageuser/View&emp_number=".$details['emp_number'];
                         ...
                         $option =  '<a href="#" class="btn empremove" rel="'.$reportid.'" title="Delete"><i class="icon-remove"></i></a>';
                        
                     $array = array($i,$details['name'],$details['order_no'],$leave_approval,$option);
                     $newarray[] = $array;
                     $i++;
                 }
             }else{
        ...
        public function actionsubordinate(){
             ...
             if ($session['user_role']!='1')
                $subordinates = $repo->getSubordinate($session['empnumber']);
             else
                $subordinates = $repo->getSubordinate($_REQUEST['emp_number']);
             ...
             if (count($subordinates)>0){
                 ...
                 {   
                        ...
                        $editurl = Yii::app()->request->baseUrl."/index.php?r=Manageuser/View&emp_number=".$empid;
                         ...
                         $option =  '<a href="#" class="btn empremove" rel="'.$empid.'" title="Delete"><i class="icon-remove"></i></a>';
