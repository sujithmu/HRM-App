...
//$eno = $_SESSION['epno'];
//echo $eno;
//if(isset($_REQUEST['rcmloginsubmit'])){
//    
//}
...
//$criteria = new HrmRoundcubeMail();
//$session = new CHttpSession;
//$session->open();
//$crit = $session['empnumber'];
//$criteria->select =  '*';
//$criteria->condition = "emp_number = $crit";
//$info = HrmRoundcubeMail::model()->findAll($criteria);

//$session = new CHttpSession;
//$session->open();
//$rcontent = HrmRoundcubeMail::model()->getroundmaillogin($session['empnumber']);
//$eno = $session['empnumber'];
//$crit = $session['empnumber'];
// try to log in

if ($_REQUEST['roundid']>0) {
   // echo "lllll".$_SESSION["epno"];
   // exit();
//    echo $_REQUEST['roundid'];
//    exit();
    
    $ctn=mysqli_connect("localhost","root","") or die("ERR: Connection");
    $databa=mysqli_select_db($ctn,"netstrat_hrm") or die("ERR: Database");
//    $pswd = mysqli_real_escape_string($ctn, $_REQUEST['_pass']);
//    $unam = mysqli_real_escape_string($ctn, $_REQUEST['_user']);
    //$eno = $_SESSION["epno"];
    //$una = $_SESSION["uname"];
    //session_start();
//    echo "ssds".$_SESSION["epno"];
//    exit();
...
//    echo "aaaa".$uname;
//    exit();
    //$sqlselect = "SELECT mail_pass FROM hrm_roundcube_mail WHERE username=";
    
    
     function decrypt($text) {
            $text = base64_decode($text);
            return mcrypt_decrypt(MCRYPT_RIJNDAEL_128, 'aaaaa', $text, MCRYPT_MODE_ECB, 'keee');
        }    
  ...
  $roundall = "SELECT username,mail_password FROM hrm_roundcube_mail WHERE id=".$_REQUEST['roundid'];
    $exe = mysqli_query($ctn, $roundall);
    $rdata = mysqli_fetch_array($exe);
   
    mysqli_close($ctn);
//    echo $rdata[username];
//    echo $rdata[mail_password];
    $passw = decrypt($rdata[mail_password]);
...
//        $round->mail_password = $_REQUEST['_pass'];
//        $session = new CHttpSession;
//        $session->open();
...
<script type="text/javascript">
 ...
 $jparent('body').removeClass("loading_round");
 var error_id = $jparent('#rmailerror');
 error_id.show();
 var round_id = $jparent('#roundpopupmodal');
 round_id.modal("show");
</script>
<?php
